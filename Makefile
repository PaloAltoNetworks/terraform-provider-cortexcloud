...
# The variables in main.go are addressed as bare `main.<Var>`, NOT as
# `<module path>/main.<Var>` — package main sits at the module root, and the
# linker resolves it by package name. The qualified form is silently ignored,
# which is why these values used to log as NOCOMMIT/UNKNOWN despite being set
# here. Symbols in real packages (sdk/version) do need the full import path.
#
# ProductVersion feeds the User-Agent sent on every API call. goreleaser sets
# it from the release tag; set it here too so locally-built providers do not
# report the "dev" default on the wire.
define LDFLAGS
-s -w \
...
-X 'main.ProviderVersion=$(PROVIDER_VERSION)' \
-X 'github.com/PaloAltoNetworks/terraform-provider-cortexcloud/sdk/version.ProductVersion=$(PROVIDER_VERSION)'
endef
