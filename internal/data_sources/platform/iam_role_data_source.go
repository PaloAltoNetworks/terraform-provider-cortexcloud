...
func (r *RoleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var config models.IamRoleDataSourceModel
	...
	if !config.RoleID.IsNull() && !config.RoleID.IsUnknown() && config.RoleID.ValueString() != "" {
		lookup = config.RoleID.ValueString()
	} else if !config.RoleID.IsNull() && !config.RoleID.IsUnknown() && config.RoleID.ValueString() != "" {
		lookup = config.RoleID.ValueString()
	} else {
		resp.Diagnostics.AddError("Invalid configuration", "Either role_id or id must be provided.")
		return
	}
	...
	for i := range listResp.Data {
		if listResp.Data[i].RoleID == lookup {
			role = &listResp.Data[i]
			break
		}
	}

	if role == nil {
		resp.Diagnostics.AddError("IAM Role not found", fmt.Sprintf("IAM Role with role_id %q not found", lookup))
		return
	}

	config.RefreshFromRemote(role)
