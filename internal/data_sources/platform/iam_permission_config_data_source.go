...
func (d *iamPermissionConfigDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	subPermissionType := types.ObjectType{
	...
	permissionType := types.ObjectType{
		AttrTypes: map[string]attr.Type{
			...
			"action_name":     types.StringType,
			"sub_permissions": types.ListType{ElemType: subPermissionType},
		},
	}

	subCategoryType := types.ObjectType{
		AttrTypes: map[string]attr.Type{
			"sub_category_name": types.StringType,
			"permissions":       types.ListType{ElemType: permissionType},
		},
	}

	rbacPermissionType := types.ObjectType{
		AttrTypes: map[string]attr.Type{
			"category_name":  types.StringType,
			"sub_categories": types.ListType{ElemType: subCategoryType},
		},
	}

	datasetGroupType := types.ObjectType{
		AttrTypes: map[string]attr.Type{
			"datasets":         types.ListType{ElemType: types.StringType},
			"dataset_category": types.StringType,
		},
	}

	rbacPermissionsList, diag := types.ListValueFrom(ctx, rbacPermissionType, rbacPermissions)
	resp.Diagnostics.Append(diag...)

	datasetGroupsList, diag := types.ListValueFrom(ctx, datasetGroupType, datasetGroups)
	resp.Diagnostics.Append(diag...)

	state.RbacPermissions = rbacPermissionsList
	state.DatasetGroups = datasetGroupsList
