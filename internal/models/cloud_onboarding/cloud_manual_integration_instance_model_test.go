...
// TestRefreshFromReadResponseKeepsUnreportedAuditCredentials proves the two
// audit members the platform never reports survive a refresh.
//
// cloudtrail_role and sqs_url are accepted on write and absent from every
// reply. Nulling them on refresh would show as drift on every subsequent plan,
// and re-sending the nulled state would drop them from the connector.
func TestRefreshFromReadResponseKeepsUnreportedAuditCredentials(t *testing.T) {
	t.Parallel()
	...
	auditLogs, diags := types.ObjectValue(auditLogTypes, map[string]attr.Value{
		...
		"cloudtrail_role":       types.StringValue("arn:aws:iam::782785052462:role/example-cloudtrail"),
		"sqs_url":               types.StringValue("https://sqs.us-east-1.amazonaws.com/782785052462/example"),
	})
	if diags.HasError() {
	...
	if refreshed["cloudtrail_role"].Equal(types.StringValue("arn:aws:iam::782785052462:role/example-cloudtrail")) == false {
		t.Errorf("cloudtrail_role = %v; the platform never reports it, so the configured value must be kept", refreshed["cloudtrail_role"])
	}
	if refreshed["sqs_url"].Equal(types.StringValue("https://sqs.us-east-1.amazonaws.com/782785052462/example")) == false {
		t.Errorf("sqs_url = %v; the platform never reports it, so the configured value must be kept", refreshed["sqs_url"])
	}

	// What the platform does report still wins.
...
func TestRefreshFromReadResponseKeepsConfiguredScopeModificationsTheReplyOmits(t *testing.T) {
	...
	configured, diags := types.ObjectValue(ManualScopeModificationsAttributeTypes(), map[string]attr.Value{
		...
		"regions":           types.ObjectNull(ManualScopeModificationsAttributeTypes()["regions"].(types.ObjectType).AttrTypes),
		"onboard_only_mode": types.BoolValue(true),
	})
	if diags.HasError() {
