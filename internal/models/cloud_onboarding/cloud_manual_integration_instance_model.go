...
type manualScopeModifications struct {
	...
	OnboardOnlyMode *bool                     `tfsdk:"onboard_only_mode"`
}
...
func ManualScopeModificationsAttributeTypes() map[string]attr.Type {
	return map[string]attr.Type{
		...
		"onboard_only_mode": types.BoolType,
	}
}
...
func (m *CloudManualIntegrationInstanceModel) collectionConfiguration(ctx context.Context, diagnostics *diag.Diagnostics) cloudOnboardingTypes.ManualCollectionConfiguration {
	...
	auditLogs := cloudOnboardingTypes.ManualAuditLogsConfiguration{
		...
		CloudTrailRole:     value.AuditLogs.CloudTrailRole,
		SQSURL:             value.AuditLogs.SQSURL,
	}
...
// unrestrictedRegions is the region scope of a connector nobody restricted.
//
// The platform requires scope_modifications.regions on every write and answers
// a request without it with
//
//	422 {"type": "missing", "loc": ["scope_modifications", "regions"],
//	     "msg": "Field required"}
//
// The attribute is Optional and Computed, so a configuration that says nothing
// about scope leaves the object null on create and unknown while planning, and
// neither carries a regions member. Supplying this in that case is what lets a
// connector be onboarded without mentioning scope at all -- which is what every
// example does.
//
// enabled=false is not a placeholder. It is the value the platform reports for a
// connector with no region restriction, so it asks for the behaviour the silent
// configuration means rather than inventing a restriction.
func unrestrictedRegions() *cloudOnboardingTypes.ScopeModificationRegions {
	return &cloudOnboardingTypes.ScopeModificationRegions{Enabled: false}
}
...
func (m *CloudManualIntegrationInstanceModel) scopeModifications(ctx context.Context, diagnostics *diag.Diagnostics) cloudOnboardingTypes.ManualScopeModifications {
	...
	if m.ScopeModifications.IsNull() || m.ScopeModifications.IsUnknown() {
		return cloudOnboardingTypes.ManualScopeModifications{Regions: unrestrictedRegions()}
	}
	...
	scopeModifications := cloudOnboardingTypes.ManualScopeModifications{
		OnboardOnlyMode: value.OnboardOnlyMode,
	}
	...
	if value.Regions != nil {
		...
		// The practitioner restricted some other dimension but said nothing
		// about regions. The member is still required on the wire.
		scopeModifications.Regions = unrestrictedRegions()
	}
...
func refreshedCollectionConfiguration(ctx context.Context, diagnostics *diag.Diagnostics, configured types.Object, collection *cloudOnboardingTypes.CollectionConfigurationRead) types.Object {
	...
	if !configured.IsNull() && !configured.IsUnknown() {
		var previous manualCollectionConfiguration
		// The object read here is the PLAN, not prior state: the refresh runs
		// straight after the create call returns. Its Optional+Computed members
		// -- is_control_tower_byob, cloudtrail_role, sqs_url -- are unknown
		// whenever the configuration does not mention them, which is every
		// non-AWS connector. Reading it with the default options aborts the
		// apply with "Received unknown value, however the target type cannot
		// handle unknown values", AFTER the connector has been created, leaving
		// it orphaned with nothing in state tracking it. Unknown must be read
		// as absent here for the same reason it is everywhere else on this
		// type.
		diagnostics.Append(configured.As(ctx, &previous, unknownMembersAsAbsent)...)
		if diagnostics.HasError() {
			return types.ObjectNull(attributeTypes)
		}
		if previous.AuditLogs != nil {
			cloudTrailRole = previous.AuditLogs.CloudTrailRole
			sqsURL = previous.AuditLogs.SQSURL
		}
	}
...
func refreshedScopeModifications(ctx context.Context, diagnostics *diag.Diagnostics, configured types.Object, modifications *cloudOnboardingTypes.ScopeModificationsRead) types.Object {
	...
	// The platform omits onboard_only_mode from a freshly created connector's
	// reply and reports it as false once an edit has been applied. Both mean
	// "not enabled", but recording the absence as null and the later reply as
	// false makes the value appear to change when nothing did -- which
	// Terraform refuses as an inconsistent result after the edit.
	//
	// Normalising the absence to false makes the two replies agree. It does not
	// invent a setting: false is what the platform itself reports for a
	// connector that only onboards nothing differently.
	onboardOnlyMode := modifications.OnboardOnlyMode
	if onboardOnlyMode == nil {
		notEnabled := false
		onboardOnlyMode = &notEnabled
	}

	value := manualScopeModifications{
		OnboardOnlyMode: onboardOnlyMode,
	}
