...
func manualTestModel(t *testing.T, attributeTypes map[string]attr.Type, withID bool) models.CloudManualIntegrationInstanceModel {
	...
	auditLogs, diags := types.ObjectValue(models.ManualAuditLogsAttributeTypes(), map[string]attr.Value{
		...
		"cloudtrail_role":       types.StringValue(testConfiguredCloudTrailRole),
		"sqs_url":               types.StringValue(testConfiguredSQSURL),
	})
	if diags.HasError() {
...
// TestManualInstanceCreateFailureWarnsAboutOrphanedConnector verifies that a
// rejected create does not claim nothing happened. The platform can leave a
// connector behind when it rejects the request, and that connector cannot be
// removed through the API, so the practitioner has to be told to look for it.
func TestManualInstanceCreateFailureWarnsAboutOrphanedConnector(t *testing.T) {
	ctx := context.Background()
	...
	if !strings.Contains(detail, "pending") {
		t.Errorf("diagnostic must warn that a connector may have been left behind in a "+
			"pending state; detail=%q", detail)
	}
...
func TestManualInstanceReadCarriesWriteOnlyFieldsThrough(t *testing.T) {
	...
	auditLogs := refreshed.CollectionConfiguration.Attributes()["audit_logs"].(types.Object)

	cloudTrailRole := auditLogs.Attributes()["cloudtrail_role"].(types.String)
	if cloudTrailRole.ValueString() != testConfiguredCloudTrailRole {
		t.Errorf("collection_configuration.audit_logs.cloudtrail_role = %q, want the configured %q: "+
	...
	sqsURL := auditLogs.Attributes()["sqs_url"].(types.String)
	if sqsURL.ValueString() != testConfiguredSQSURL {
		t.Errorf("collection_configuration.audit_logs.sqs_url = %q, want the configured %q: "+
			"the platform never reports this field, so nulling it produces a permanent diff",
